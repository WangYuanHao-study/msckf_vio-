...
void Feature::generateInitialGuess(
  ...
  Eigen::Vector3d m = T_c1_c2.linear() * Eigen::Vector3d(z1(0), z1(1), 1.0); //将第一帧观测转换到最后一帧相机坐标系下
  ...
  // Solve for the depth.求解最小二乘问题，得到深度，并计算出一个3D位置
...
bool Feature::checkMotion(
  ...
  const StateIDType& first_cam_id = observations.begin()->first;  //这个特征点在第一次被观测到时的相机id
  const StateIDType& last_cam_id = (--observations.end())->first; //这个特征点在最后一次被观测到时的相机id

  Eigen::Isometry3d first_cam_pose; //这个特征点在第一次被观测到时的相机位姿
  ...
  Eigen::Isometry3d last_cam_pose;  //这个特征点在最后一次被观测到时的相机位姿
  ...
  //* 该特征点在第一次被观测到时和最后一次被观测到时的相机位置之差
...
bool Feature::initializePosition(
  ...
  for (auto& m : observations) {
    ...
    auto cam_state_iter = cam_states.find(m.first);  //遍历所以有观测到该特征点的相机
    if (cam_state_iter == cam_states.end()) continue;  //该观测对应的相机状态已经被滑窗移除

    // Add the measurement.//*如果相机在滑窗内，则添加观测
    measurements.push_back(m.second.head<2>());   //左目相机的观测
    measurements.push_back(m.second.tail<2>());   //右目相机的观测
  ...
  for (auto& pose : cam_poses)
    pose = pose.inverse() * T_c0_w;  //将所有相机位姿转换到第一个观测该特征点的相机坐标系下

  // Generate initial guess
  Eigen::Vector3d initial_position(0.0, 0.0, 0.0);
  generateInitialGuess(cam_poses[cam_poses.size()-1], measurements[0],
      measurements[measurements.size()-1], initial_position);  //生成初始猜测值
  Eigen::Vector3d solution(
      initial_position(0)/initial_position(2),
      initial_position(1)/initial_position(2),
      1.0/initial_position(2));    //将3D位置转换为逆深度表示
