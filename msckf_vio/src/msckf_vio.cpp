...
#if 1
static const int p_max_cnt = 50000;
static const int p_cols = 8;
static double *p_log_data = new double[p_max_cnt * p_cols];
static int p_cnt = 0;
void mylog()
{
    FILE* fp = fopen("/home/wyh/Msckf_vio/result/vio.txt","w");
    int k = 0;
    while(1)
    {
        if(k < p_cnt)
...
void MsckfVio::imuCallback(
  ...
  if (!is_gravity_set) {
    if (imu_msg_bu ffer.size() < 200) return;
...
/**
 * @brief imu 初始化（初始化零偏和重力）
 * 对 imu 数据进行平均，得到零偏和重力方向 
 * 利用角速度的均值得到零偏
 * 利用加速度的模长初始化重力大小，并计算 imu 到世界坐标系的旋转（这块其实就是个简单的加速度计调平）
 * 这里默认的 yaw 角度为 0
 */
...
void MsckfVio::featureCallback(
  ...
  if (is_first_img) {
    is_first_img = false;
    state_server.imu_state.time = msg->header.stamp.toSec();  // 这一部分使用的相机的第一帧的时间初始化状态的时间
  }
  ...
  /*
  !第一次进入时，batchImuProcessing函数中并没有任何处理，
  !因为imu_msg_buffer中存储的imu数据的时间戳都小于等于state_server.imu_state.time
  */
  batchImuProcessing(msg->header.stamp.toSec());  //使用的是当前的图像时间戳
...
void MsckfVio::batchImuProcessing(const double& time_bound) {
  ...
  for (const auto& imu_msg : imu_msg_buffer) {
    ...
    if (imu_time < state_server.imu_state.time) {  //imu的时间在当前状态的时间之前，数据太旧，丢弃
      ++used_imu_msg_cntr;
      continue;
    }
    if (imu_time > time_bound) break;  // imu的时间在当前状态的时间之后，数据太新，丢弃
...
void MsckfVio::processModel(const double& time,
  ...
  double dtime = time - imu_state.time;     //相邻两帧的imu的时间间隔
  ...
  // TODO ：这部分是可观性约束的处理，可以后续移植到算法之中 OC-EKF
  Matrix3d R_kk_1 = quaternionToRotation(imu_state.orientation_null);  //获得上一帧的姿态
  Phi.block<3, 3>(0, 0) =
    quaternionToRotation(imu_state.orientation) * R_kk_1.transpose();  //使用姿态修正，从而减少线性化的误差

  Vector3d u = R_kk_1 * IMUState::gravity;  //计算不可观子空间的基向量
  RowVector3d s = (u.transpose()*u).inverse() * u.transpose();  

  Matrix3d A1 = Phi.block<3, 3>(6, 0);
  Vector3d w1 = skewSymmetric(
      imu_state.velocity_null-imu_state.velocity) * IMUState::gravity;
  Phi.block<3, 3>(6, 0) = A1 - (A1*u-w1)*s;   //修正姿态误差对速度误差的影响

  Matrix3d A2 = Phi.block<3, 3>(12, 0);
  Vector3d w2 = skewSymmetric(
      dtime*imu_state.velocity_null+imu_state.position_null-
      imu_state.position) * IMUState::gravity;
  Phi.block<3, 3>(12, 0) = A2 - (A2*u-w2)*s;   //修正姿态误差对位置误差的影响
  ...
  if (state_server.cam_states.size() > 0) {   //每一次imu预积分之后都要更新协方差
  ...
  MatrixXd state_cov_fixed = (state_server.state_cov +    //进行对称化处理，防止数值误差导致协方差矩阵不对称
...
void MsckfVio::stateAugmentation(const double& time) {
  //外参的初始值的大小是通过配置文件得到初始值的
  const Matrix3d& R_i_c = state_server.imu_state.R_imu_cam0;
  const Vector3d& t_c_i = state_server.imu_state.t_cam0_imu;

  // Add a new camera state to the state server.
  Matrix3d R_w_i = quaternionToRotation(     //四元数转旋转矩阵，工程上的基本操作
      state_server.imu_state.orientation);
  Matrix3d R_w_c = R_i_c * R_w_i;            //相机坐标系到世界坐标系的姿态
  Vector3d t_c_w = state_server.imu_state.position +      //相机坐标系到世界坐标系的位置
    R_w_i.transpose()*t_c_i;

  state_server.cam_states[state_server.imu_state.id] =   //为当前帧的IMU状态添加相机状态
    CAMState(state_server.imu_state.id);
  CAMState& cam_state = state_server.cam_states[
    state_server.imu_state.id];

  cam_state.time = time;   //相机状态的时间还真就是相机的时间
  cam_state.orientation = rotationToQuaternion(R_w_c);  //*相机的状态用cam到世界坐标系的旋转表示
  cam_state.position = t_c_w;   //*相机的状态用cam到世界坐标系的位置表示
...
//  TODO 这里对特征点的管理可以借鉴一下
void MsckfVio::addFeatureObservations(
    const CameraMeasurementConstPtr& msg) {

  StateIDType state_id = state_server.imu_state.id;
  int curr_feature_num = map_server.size();  // *第一次执行的时候，为0
  ...
  for (const auto& feature : msg->features) {
    if (map_server.find(feature.id) == map_server.end()) {
      ...
      map_server[feature.id].observations[state_id] =   //左目右目的特征点的坐标都有存储
  ...
  // Compute the feature tracking rate.
  tracking_rate =
...
void MsckfVio::featureJacobian(
  ...
  // *针对每个特征点为每个观测都计算一个4x6的H_xi和4x3的H_fi（这块和MSCKF那些事是保持一致的）
  ...
  // of H_fj. 通过对H_fj进行SVD分解得到左零空间投影
...
void MsckfVio::measurementUpdate(
  ...
  if (H.rows() > H.cols()) {  // *如果量测方程的数量大于状态变量的数量
  ...
  //TODO 对外参进行更新，后续可以仿照这个对我们的算法进行更改
...
bool MsckfVio::gatingTest(
  ...
  if (gamma < chi_squared_test_table[dof]) {  //进行卡方检验筛去异常值
    //cout << "passed" << endl;
    return true;
  } else {
    //cout << "failed" << endl;
...
void MsckfVio::removeLostFeatures() {
  ...
  for (auto iter = map_server.begin();
    ...
    // Pass the features that are still being tracked. 跳过还在被跟踪的特征点
    if (feature.observations.find(state_server.imu_state.id) !=  //在当前帧存在观测
        feature.observations.end()) continue;
    if (feature.observations.size() < 3) {        //存在特征点的观测数小于3，直接丢弃，三角化不出来
    ...
    // has not been.
    if (!feature.is_initialized) {   //如果特征点没有被初始化
      if (!feature.checkMotion(state_server.cam_states)) {    //特征点视差不够，无法初始化，丢弃
        invalid_feature_ids.push_back(feature.id);  //丢弃该特征点
        continue;
      } else {
    ...
    //TODO 这块考虑到了可观性的问题，所以对应的雅可比矩阵和《MSCKF那些事存在出入》
    jacobian_row_size += 4*feature.observations.size() - 3; //减去3是在特征点三角化的时候会将三维自由度投影到零空间
  ...
  MatrixXd H_x = MatrixXd::Zero(jacobian_row_size,   //*这块会改变矩阵的维度
...
void MsckfVio::findRedundantCamStates(
  ...
  for (int i = 0; i < 4; ++i)
    --key_cam_state_iter;
  auto cam_state_iter = key_cam_state_iter;  //?关键帧选择倒数第四帧
  ++cam_state_iter;
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    double distance = (position-key_position).norm();  //* 计算连续两帧之间的平移距离
    double angle = AngleAxisd(
        rotation*key_rotation.transpose()).angle();    //* 计算连续两帧之间的旋转角度

    if (angle < rotation_threshold &&
        distance < translation_threshold &&
        tracking_rate > tracking_rate_threshold) {    //* 如果两帧之间的运动幅度不大，并且跟踪率也不错，就可以删掉
...
void MsckfVio::pruneCamStateBuffer() {
  ...
  if (state_server.cam_states.size() < max_cam_state_size) //相机的状态数没有超过上限
    return;
